...
export const Fashion = ({onClickMe = f => f}) => {
   ...
   useEffect(() => {
      ...
      setLoading(true)
      axios(`https://api.nytimes.com/svc/topstories/v2/fashion.json?api-key=${key}`)
         .then(result => {
         ...
         <div key={i} className={`flex flex-col md:border items-center overflow-hidden`}>
            <figure className='order-2'>
               ...
               <figcaption className='px-5 pb-6'>
                  <p className='mb-3 text-gray-600'>{timeFunc(item.published_date)}</p>
                  <a href={item.url} target="_blank" className='text-2xl cursor-pointer font-bold'>{item.title}</a>
